...
function [polygonX polygonY] = makePolygon_image(Profile, r, maxR, numPetals, minFeatureSize, a, useFullOcculter)
...
if (sum(notClose) == 0)
    ...
    if (sum(isAt0or1) == length(isAt0or1)) && (Profile(1) == 1) && (isempty(switchOver) || switchOver == 1)
       ...
       error('Error: occulter structure is too disconnected to model with makePolygon_image.')
    end
else
    if rr(1) ~= a
        rr = [a rr];
